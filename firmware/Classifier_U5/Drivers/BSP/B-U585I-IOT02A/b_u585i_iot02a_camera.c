...
DMA_HandleTypeDef hdma_handler; /* exposed for stm32u5xx_it.c IRQ handler */
...
int32_t BSP_CAMERA_Init(uint32_t Instance, uint32_t Resolution, uint32_t PixelFormat)
{
  ...
  else
  {
    ...
    if (BSP_MX_DCMI_Init(&hcamera_dcmi) != HAL_OK)
    {
...
/**
  ...
  * @retval HAL status
  */
__weak HAL_StatusTypeDef BSP_MX_DCMI_Init(DCMI_HandleTypeDef *hdcmi)
{
...
static void DCMI_MspInit(DCMI_HandleTypeDef *hdcmi)
{
  ...
  /* PATCH: original BSP inserts a second node (DCMINode2) here for a
   * "ping-pong" capture that writes both nodes to the same buffer.
   * That's broken because HAL_DCMI_Start_DMA only patches the Head
   * node's CSAR/CDAR/CBR1 — Node 2 keeps the default-initialised
   * (zeroed) addresses, so after Frame 1's TC the chain wraps to
   * Node 2 with src=0 dst=0 and the channel raises USE/DTE.
   *
   * Single-node circular queue: the node's NEXTNODE pointer references
   * itself, HAL_DCMI_Start_DMA patches that node correctly, and TC
   * fires once per frame. */
  (void)DCMINode2;  /* keep symbol referenced */
