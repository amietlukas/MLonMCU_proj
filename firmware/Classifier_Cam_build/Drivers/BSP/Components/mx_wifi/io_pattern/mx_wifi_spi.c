...
static int8_t mx_wifi_spi_txrx_start(void)
{
  ...
  /* If FLOW is already HIGH at startup (e.g. EMW3080 application was already
   * running before the SPI driver got here), no rising edge will fire, so
   * SEM_WAIT(SpiFlowRiseSem) inside wait_flow_high() would block until
   * timeout. Pre-signal the semaphore in that case. */
  if (!MX_WIFI_SPI_FLOW_IS_LOW())
  {
    SEM_SIGNAL(SpiFlowRiseSem);
  }
